...
class Bet(object):
    ...
    def calculate(self, balance: int, title: str = None) -> dict:
        ...
        if title is None:
            ...
            for event in self.settings.events:
                if isinstance(event.title, str) and re.search(event.title, title, flags=event.regex_flags) or callable(event.title) and event.title(title):
                    if event.filter is not None and not event.filter(self):
                        continue
                    if self.event_strategy(balance, event.event_chances, event.max_points, event.strict, event.regex_flags):
                        is_event = True
                        break
        ...
        if not is_event:
            ...
            if any(outcome[OutcomeKeys.TOTAL_POINTS] == 0 for outcome in self.outcomes):
                self.decision["choice"] = random.choice(list(filter(lambda i: self.outcomes[i][OutcomeKeys.TOTAL_POINTS] == 0, range(len(self.outcomes)))))
    ...
    def event_strategy(self, balance: int, event_chances: dict[str, float], max_points: int, strict: bool, regex_flags: re.RegexFlag) -> bool:
        failed_logger_extra = {"color": Settings.logger.color_palette.BET_FAILED}

        if strict and len(self.outcomes) != len(event_chances):
            logger.info("Event outcomes counts don't match", failed_logger_extra)
            return False

        total_chance = 0
        outcome_chances = [0.0] * len(self.outcomes)
        available_event_chances = set(event_chances.keys())

        for i, decision_outcome in enumerate(self.outcomes):
            for event_outcome, chance in event_chances.items():
                if event_outcome in available_event_chances and re.search(event_outcome, decision_outcome["title"], flags=regex_flags):
                    if strict:
        ...
        if strict and len(available_event_chances) > 0:
            logger.info("Event outcomes don't match", failed_logger_extra)
            return False

        outcome_chances = [chance / total_chance for chance in outcome_chances]
        max_bet, max_ammount = None, 0
        ...
        for i, outcome in enumerate(self.outcomes):
            if outcome[OutcomeKeys.TOTAL_POINTS] / self.total_points < outcome_chances[i]:
                ...
                bet_ammount = (math.sqrt((c * p + o - p) ** 2 - 4 * (c * o * p - p * t)) - c * p - o + p) / 2

                if bet_ammount > max_ammount:
                    max_bet = i
                    max_ammount = bet_ammount

        self.decision["choice"] = max_bet
        self.decision["amount"] = max_ammount

        if max_points is not None:
            self.decision["amount"] = min(self.decision["amount"], max_points * outcome_chances[i])
